...
#pragma region behavior_tree_nodes
    void BehaviorTreeNode::DeleteChildren() {
    ...
    void BehaviorTreeNode::ResetChildren() {
        for each (auto node in Children)
        {
    ...
    void BehaviorTreeNode::AssignBot(GooseBot* _bot) {
        bot = _bot;
        for each (auto child in Children)
        {
            child->AssignBot(_bot);
        }
    }

    void NodeRepeatWhileTrue::Execute() {
        if (Children.empty()) {
            Status = NodeStatus::False;
            return;
        }

        BehaviorTreeNode* child = Children[0];
        switch (child->Status)
        {
        case NodeStatus::True: {
            Status = NodeStatus::Running;
            ResetChildren();
            child->Execute();
        }
        case NodeStatus::False: {
            Status = NodeStatus::False;
            ResetChildren();
        }
    ...
    void NodeRepeat::Execute() {
        ...
        BehaviorTreeNode* child = Children[0];
        if (child->Status == NodeStatus::True || child->Status == NodeStatus::False)
        {
            ResetChildren();
            child->Execute();
        }
        else if (child->Status == NodeStatus::Running || child->Status == NodeStatus::New)
        {
            child->Execute();
        }
    }

    void NodeAny::Execute() {
        if (Children.empty()) {
            Status = NodeStatus::False;
            return;
        }
        ...
        for each (auto child in Children)
        {
            if (child->Status == NodeStatus::True)
            {
    ...
    void NodeEvery::Execute() {
        ...
        for each (auto child in Children)
        {
            if (child->Status == NodeStatus::False)
            {
                Status = NodeStatus::False;
                ResetChildren();
                return;
    ...
    void NodeNot::Execute() {
        ...
        else if (child->Status == NodeStatus::False)
        {
            Status = NodeStatus::True;
            ResetChildren();
            return;
    ...
    // Leaf nodes
    void NodeAccrueEconomy::Execute() {
        // TODO(gucarva)
        Status = NodeStatus::True;
    }

    void NodeAccrueProduction::Execute() {
        // TODO(gucarva)
        Status = NodeStatus::True;

    }

    void NodeAccrueInformation::Execute() {
        // TODO(gucarva)
        Status = NodeStatus::True;

    }

    void NodeAccrueArmy::Execute() {
        // TODO(gucarva)
        Status = NodeStatus::True;

    }

    void NodeAccrueForce::Execute() {
        // TODO(gucarva)
        Status = NodeStatus::True;
    }

    void NodeMoveRandomly::Execute() {
        Status = NodeStatus::Running;

        static uint32_t lastGameLoop = 0;
        uint32_t thisGameLoop = bot->Observation()->GetGameLoop();

        if (thisGameLoop <= lastGameLoop + 22)
        {
            return;
        }
        lastGameLoop = thisGameLoop;

        const sc2::ObservationInterface* observation = bot->Observation();
        sc2::ActionInterface* action = bot->Actions();

        sc2::Units myUnits = observation->GetUnits(sc2::Unit::Alliance::Self);
        if (myUnits.empty()) {
            Status = NodeStatus::False;
            return;
        }
        ...
        sc2::Units myWorkers;
        for (auto u : myUnits) {
            if (u->unit_type == sc2::UNIT_TYPEID::ZERG_DRONE)
            {
                myWorkers.push_back(u);
            }
        }
        ...
        const sc2::Unit* unit = sc2::GetRandomEntry(myWorkers);

        sc2::Point2D moveTarget = sc2::FindRandomLocation(observation->GetGameInfo());
        if (!bot->CanPathToLocation(unit, moveTarget)) {
            Status = NodeStatus::False;
            return;
        }

        action->UnitCommand(unit, sc2::ABILITY_ID::SMART, moveTarget);
        Status = NodeStatus::True;

        auto time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
        std::cout << ctime(&time) << " moved unit " << unit->tag << std::endl;
    }

#pragma endregion All of the behavior tree nodes are defined here.

#pragma region my_main

    BehaviorTreeNode* GooseBot::BuildOrderMoveRandomly() {
        BehaviorTreeNode *root = new NodeRepeat();
        BehaviorTreeNode *moveRand = new NodeMoveRandomly();

        root->Children.push_back(moveRand);

        return root;
    }

    void GooseBot::Initialize() {
        if (Initialized) {
            return;
        }
        Initialized = true;

        // Choose a build order and roll with it.
        Root = BuildOrderMoveRandomly();
        Root->AssignBot(this);
    }

    void GooseBot::Heartbeat() {
        if (!Initialized) {
            Initialize();
        }
        Root->Execute();
    ...
    bool GooseBot::CanPathToLocation(const sc2::Unit* unit, sc2::Point2D& target_pos) {
...
#pragma endregion All of my code, top kek
...
#pragma endregion All of the default bot logic is here.
}
